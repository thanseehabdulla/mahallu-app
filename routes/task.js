...
router.put('/updatemembersstatus/:id',function(req,res,next){
    var memberstatus = req.body['memberstatus'];
    var updated_at = req.body['updated_at'];
    db.collection('users').update({_id: ObjectID(req.params.id)},{$set:{memberstatus:memberstatus,updated_at:updated_at}} ,function (err, results) {
                if (err) throw err
                res.json(results);
            })});
